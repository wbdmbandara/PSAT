...
#1 @admin_bp.route("/report/<report_type>", methods=["GET"])
...
#2 @admin_bp.route("/report/<report_type>", methods=["GET"])
# def view_report(report_type):
#     if "user_id" not in session:
#         return redirect(url_for("admin.login"))
...
#             # Email/Campaign name filter
#             if email_filter:
#                 base_query += " AND (campaign_name LIKE %s OR template_name LIKE %s)"
#                 query_params.extend([f"%{email_filter}%", f"%{email_filter}%"])
            
#             # Date From filter (using created_at if you have that column, otherwise remove)
#             if date_from:
